  ...
  return (
    <form action={action} className="flex flex-col gap-3">
      <div className="flex flex-col gap-2">
        ...
        <Input id="titre" name="titre" required minLength={2} placeholder={t("ex : Présentation de l'offre")} />
      </div>

      <div className="grid grid-cols-2 gap-2">
        <div className="flex flex-col gap-2">
          <Label htmlFor="dateDebut">{t("De")}</Label>
          <Input id="dateDebut" name="dateDebut" type="datetime-local" required />
        </div>
        <div className="flex flex-col gap-2">
          <Label htmlFor="dateFin">{t("Au")}</Label>
      ...
      <div className="flex gap-2">
        <Button type="submit" size="sm" disabled={enCours}>
          ...
          {enCours ? t("Création…") : t("Ajouter la réunion")}
        </Button>
