...
import { Building2, Phone, ArrowRightCircle, FileText, ClipboardList } from "lucide-react";
import { avecEntreprise } from "@/db/client";
import { deal, contact, compteClient, devis, facture, bonCommandeVente, historiqueStatutDeal, utilisateur } from "@/db/schema";
import { recupererUtilisateurConnecte } from "@/lib/session";
import { peut } from "@/lib/permissions";
import { idsVisibles } from "@/lib/portee";
import { STATUT_DEAL, STATUT_DEVIS, STATUT_FACTURE, STATUT_BON_COMMANDE_VENTE } from "@/lib/libelles";
...
export default async function PageFicheDeal({ params }: { params: Promise<{ id: string }> }) {
  ...
  const donnees = await avecEntreprise(utilisateurConnecte.entrepriseId, async (tx) => {
    ...
    const [[leContact], compte, devisListe, facturesListe, bonsCommandeListe, historique] = await Promise.all([
  ...
  if (!donnees) notFound();
  const { fiche, contact: leContact, compte, devisListe, facturesListe, bonsCommandeListe, historique } = donnees;
