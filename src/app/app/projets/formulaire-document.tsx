    ...
    <form action={action} className="flex flex-col gap-3 rounded-lg border p-4">
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2 items-end">
        <div className="flex flex-col gap-1">
          <Label htmlFor="fichier">{t("Fichier")}</Label>
        ...
        {autoriserSensible ? (
          <div className="flex flex-col gap-1">
            ...
            <Select id="categorie" name="categorie" value={categorie} onChange={(e) => setCategorie(e.target.value)} className="w-44">
              <option value="GENERAL">{t("Général")}</option>
              <option value="PIECE_IDENTITE">{t("Pièce d'identité")}</option>
              <option value="DONNEES_SANTE">{t("Données de santé")}</option>
              <option value="AUTRE_SENSIBLE">{t("Autre sensible")}</option>
            </Select>
          </div>
        ) : null}
      </div>
      ...
      <Button type="submit" size="sm" disabled={enCours} className="self-start">
        ...
        {enCours ? t("Téléversement…") : t("Ajouter")}
      </Button>
    </form>
  );
}
