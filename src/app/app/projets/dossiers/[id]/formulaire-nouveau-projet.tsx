...
export function FormulaireNouveauProjet({ dossierId, vocab }: { dossierId: string; vocab: VocabulaireEntree }) {
  ...
  if (!ouvert) {
    return (
      <Button variant="outline" size="sm" onClick={() => setOuvert(true)}>
        ...
        {t("Nouveau {objet}", { objet: t(vocab.singulier).toLowerCase() })}
      </Button>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("Nouveau {objet}", { objet: t(vocab.singulier).toLowerCase() })}</CardTitle>
      </CardHeader>
      <CardContent>
        <form action={action} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="dateEcheance">{t("Échéance")}</Label>
