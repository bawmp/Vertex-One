...
export function PanneauPersonnes({ collegues: initiaux, dmParCollegue, canalActifId }: { collegues: Collegue[]; dmParCollegue: DmParCollegue; canalActifId: string | null }) {
  ...
  if (collegues.length === 0) return <p className="px-2.5 text-xs text-muted-foreground">Vous êtes seul(e) pour l&apos;instant. Invitez des collègues dans Paramètres → Équipe.</p>;
  ...
  return (
    <nav aria-label="Messages directs" className="flex flex-col gap-0.5">
      {ordonnes.map((c) => {
        ...
        return (
          ...
          >
            <span className="flex min-w-0 items-center gap-2">
              <span className={`size-2.5 shrink-0 rounded-full ${c.enLigne ? "bg-emerald-500" : "bg-muted-foreground/30"}`} role="img" aria-label={c.enLigne ? "En ligne" : "Hors ligne"} />
