...
export function FormulaireNouveauGroupe({ collegues }: { collegues: Pick<Collegue, "id" | "nom">[] }) {
  ...
  return (
    <form action={action} className="flex flex-col gap-2 rounded-lg border border-border p-2">
      <Input name="nom" placeholder="Nom du groupe" required minLength={2} maxLength={60} autoFocus aria-label="Nom du groupe" />
      <fieldset className="flex max-h-44 flex-col gap-0.5 overflow-y-auto">
        <legend className="mb-1 text-xs text-muted-foreground">Inviter (visible de ses seuls membres)</legend>
        {collegues.length === 0 ? <p className="text-xs text-muted-foreground">Aucun collègue à inviter pour l&apos;instant.</p> : null}
        {collegues.map((c) => (
      ...
      <div className="flex gap-2">
        <Button type="submit" size="sm" disabled={enCours}>
          {enCours ? <Spinner /> : "Créer le groupe"}
        </Button>
