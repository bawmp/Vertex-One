...
const date = (iso: string) => new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium", timeZone: "Africa/Douala" }).format(new Date(iso));
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="relative">
        ...
        <Input value={saisie} onChange={(e) => setSaisie(e.target.value)} placeholder="Rechercher un message…" maxLength={100} aria-label="Rechercher dans les messages" className="pl-8 pr-8" />
        {saisie ? (
          <button type="button" onClick={() => setSaisie("")} aria-label="Effacer la recherche" className="absolute right-2 top-1/2 -translate-y-1/2">
      ...
      {resultats ? (
        <div role="region" aria-label="Résultats de la recherche" className="flex max-h-80 flex-col overflow-y-auto rounded-lg border border-border bg-card">
          {resultats.length === 0 ? (
            <p className="p-3 text-sm text-muted-foreground">Aucun message ne correspond.</p>
          ) : (
            resultats.map((r) => (
              <Link
                key={r.messageId}
              ...
              >
                <span className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                  <span className="truncate font-medium text-foreground">{nomsCanaux[r.canalId] ?? "Canal"}</span>
                  <span className="shrink-0">{date(r.creeLe)}</span>
                </span>
