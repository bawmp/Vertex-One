  ...
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
      ...
      <div className="flex items-center gap-1">
        <Button type="button" variant="ghost" size="icon-sm" aria-label="Renommer" onClick={() => setEnEdition(true)}>
          <Pencil aria-hidden />
        </Button>
        <Button
          type="button"
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-sm font-medium text-muted-foreground">Départements</h2>
        {!ouvert ? (
      ...
      {ouvert ? (
        <form action={action} className="mb-3 flex items-center gap-2 rounded-lg border p-3">
          <Input name="nom" placeholder="ex : Ventes, Support, Comptabilité" required className="h-8" autoFocus />
          <Button type="submit" size="sm" disabled={enCours}>
      ...
      <Card className="p-0">
        <div className="flex flex-col divide-y divide-border">
          ...
          {services.length === 0 ? (
            <p className="px-4 py-6 text-center text-sm text-muted-foreground">Aucun département pour le moment.</p>
          ) : null}
        </div>
      </Card>
    </div>
