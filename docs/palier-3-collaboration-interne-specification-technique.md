...
## 11. Écarts réels avec ce document, constatés à la construction

- **`entrepriseId` ajouté sur `JournalAccesDocument`**, absent du sketch initial (qui ne portait que `documentId`) — même raisonnement que `Tache`/`Commentaire` au Palier 2. Une raison supplémentaire ici : le journal doit rester lisible même si le `Document` sensible auquel il se réfère est un jour réellement effacé (droit à l'effacement, section 9) — une FK stricte vers `document` avec suppression en cascade aurait justement empêché de conserver la trace de cette suppression.
- **Le prestataire de chat et Cloudflare R2 sont implémentés en code réel mais restent des stubs documentés** tant que `STREAM_CHAT_API_KEY`/`R2_ACCOUNT_ID` etc. ne sont pas configurés — même traitement que Migadu/NotchPay/Resend avant leur configuration (voir CLAUDE.md) : la convention de préfixage, la table `Canal`, le pont Projet→Canal, l'upload/téléchargement/effacement R2 fonctionnent tous réellement, seul l'appel réseau final est différé.
- **`CHAT_INTERNE` (déjà existant dans `src/lib/plans.ts` pour un usage plus large) sert de verrou unique pour les trois sous-modules** (Messagerie, Documents, Annonces), exactement comme l'exemple de code de la section 8 le suggère, plutôt que d'introduire trois clés `Fonctionnalite` séparées.
- **Aucune fenêtre de chat temps réel construite côté produit** : la page `/app/messagerie` liste les canaux visibles selon la portée (section 7) mais affiche un message explicite tant que le prestataire n'est pas configuré, plutôt qu'une interface de discussion qui ne pourrait de toute façon rien afficher.
